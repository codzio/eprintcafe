...
class PhysicalOrders extends Controller {
	...
	public function add(Request $request) {
		...
		$productList = ProductModel::where('is_active', 1)->where('product_type', 'digital')->get();
			        	...
			        	if ($isOrderCreated) {
			        		...
			        		if (!empty($getSavedCartItem) && $getSavedCartItem->count()) {
			        			...
			        			foreach ($getSavedCartItem as $cartData) {
					    			...
					    			$productName = ProductModel::where('id', $cartData->product_id)->where('product_type', 'digital')->value('name');
			        			...
			        			$productName = ProductModel::where('id', $productId)->where('product_type', 'digital')->value('name');
			        	...
			        	if (!empty($isPincodeExist) && $isPincodeExist->count()) {
			        		...
			        		*/
			        		...
			        		if ($action == 'calculate') {
				        		...
				        		$productName = ProductModel::where('id', $request->post('product'))->where('product_type', 'digital')->value('name');
